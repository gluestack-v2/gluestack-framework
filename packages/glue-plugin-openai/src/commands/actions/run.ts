...
export default async (app: AppCLI, prompt: string): Promise<void> => {
  ...
  const plugin: GlueStackPlugin = app.getPluginByName(
    pluginName
  ) as GlueStackPlugin;
  if (!plugin) {
  ...
  if (!dictionary) {
    error(
      `Plugin "${pluginName}"`,
      `dictionary file at location "${relative('.', dictionary)}" not found`
    );
  ...
  if (!(await fileExists(envfilepath))) {
    error(
      `Plugin "${pluginName}"`,
      ` env file at location "${relative('.', envfilepath)}" not found`
    );
    process.exit(1);
  }
  ...
  if (
    ...
    process.env.OPENAI_API_KEY.endsWith('xxxx')
  ) {
    error(
      `Plugin "${pluginName}"`,
      `OPENAI_API_KEY at location "${relative('.', envfilepath)}" is invalid`
    );
    process.exit(1);
  }
...
const saveResponseToWebPlugin = async (
  app: AppCLI,
  prompt: string,
  response: any
) => {
  ...
  if (!(await fileExists(filepath))) {
    error(
      ...
      `file at location "${relative('.', filepath)}" not found`
    );
    process.exit(1);
  }
  ...
  success(
    `Instance "${instance.getName()}"`,
    `saved response to ${relative('.', filepath)}`
  );
};
